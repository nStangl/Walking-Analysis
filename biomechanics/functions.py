...
def load_pickles(folder="pickles", files=None):
    ...
    # everything in pickles directory
        ...
        # Test to make sure folder exists
        assert os.path.isdir(
            folder
        ), f"Could not find folder {folder}. Make sure you specified the correct path."

        pickle_files = glob.glob(f"{folder}/*.pkl")
        if len(pickle_files) == 0:
            print(f"WARN: No pickle files found in {folder}.")
...
def get_step_metrics(
    tracklets, video, person_id, dim, joint="Heel", smoothing=False, _print=False
):
    """
...
def get_asymmetry(
    tracklets, video, person_id, dim, joint="Hip", smoothing=False, _print=False
):
    """
    ...
    asymmetry = 1 - (
        np.trapz(r) / np.trapz(l)
    )  # > 0: asymmetry towards right, < 0: asymmetry towards left
...
def process_biometrics_df(folder="pickles"):
    ...
    for video in tracklets_dict:
        ...
        for person_id in range(n_people):
            ...
            steps_length, avg_step_length, speed, time, distance = get_step_metrics(
                tracklets_dict,
                video,
                person_id=person_id,
                dim=dim_step,
